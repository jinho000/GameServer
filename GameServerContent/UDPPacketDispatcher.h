...
{
	using PacketHandler = std::function<void(std::shared_ptr<UDPSession>, std::shared_ptr<ServerPacketBase>, const EndPoint&)>;
...
public:
	...
	static void ProcessHandler(PtrSUDPSession _session, std::shared_ptr<ServerPacketBase> _packet)
	{
		// 패킷 변환
